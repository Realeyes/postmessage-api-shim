...
export function ReleaseClient(client) {
    return new Promise((resolve) => {
        ...
        const events = Object.keys(client._events); //FIXME: relying on private property...
        let unsubscriptions = [];
        events.forEach(ev => {
            unsubscriptions.push(client.off(ev));
        });
        Promise.all(unsubscriptions).then(() => {
...
export function CreateClientAsync(clientWindow, serverWindow, timeout = 30000, token = 'defaultToken') {
    return new Promise( (resolve, reject) => {
        let handler = ({data}) => {
            var serverMethods = data.payload;
