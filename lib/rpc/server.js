...
export function CreateServer(serverWindow, api, token = 'defaultToken') {
    ...
    serverWindow.addEventListener('message', ({ data: { id, method, args = [], _token }, source }) => {
        ...
        if (method === 'off') {
            if (args.length > 0) { // remove all listener of the client for a specific event
                const eventName = args[0];
                const handlers = clientEventHandlers.get(source, eventName);
                handlers.forEach(h => {
                    api.removeListener(eventName, h);
                });
                clientEventHandlers.delete(source, eventName);
            }
            else { // remove all listener of the client
                const events = clientEventHandlers.get(source);
                Object.keys(events).forEach(eventName => {
                    events[eventName].forEach(h => {
                        api.removeListener(eventName, h);
                    });
                })
            }
