 ...
 * @param {string} token
 */
export default function (serverWindow, api, token) {
    serverWindow.addEventListener('message', ({ data: { id, method, args = [], _token }, source }) => {
        // ignore messages that are not sent from the client with a correct token
        if (_token !== token) {
            return;
        }

        // call to nonexistent method returns an error
        // todo: should think on an error object format, if a string is sent it gets spread in a receiver
        ...
        if (method === 'on') {
            ...
            return post(source, id, message.MESSAGE_TYPE_RESPONSE, token);
        }
